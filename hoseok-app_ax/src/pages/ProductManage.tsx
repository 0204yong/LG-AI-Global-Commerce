    ...
    try {
      ...
      const res = await fetch('/api/products', {
        ...
        body: JSON.stringify({
          ...
          website_id: storeId === 'ALL' ? 'US' : storeId,
          translations: [
      ...
      <section className="admin-panel add-product-panel">
        ...
        <form onSubmit={handleSubmit} className="product-form">
          <div className="form-group">
            ...
            <input type="text" value={form.sku} onChange={e => setForm({...form, sku: e.target.value})} placeholder="e.g. PRD-A001" />
          </div>
          <div className="form-group">
              ...
              <tbody>
                {products.length === 0 ? (
                  ...
                  products.map(p => (
                    <tr key={p.id}>
                      <td>#{p.id}</td>
                      <td style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>{p.sku || '-'}</td>
                      <td className="fw-bold">{p.name}</td>
                      <td>${p.price.toLocaleString()}</td>
                      <td>
                        <span className={p.stock_quantity > 10 ? 'badge-success' : 'badge-warning'}>
                          {p.stock_quantity}
                        </span>
                      </td>
