              ...
              <tbody>
                {data.recentOrders.length === 0 ? (
                  ...
                  data.recentOrders.map((o: any) => (
                    <tr key={o.id}>
                      <td style={{ fontSize: '0.8rem', color: 'var(--text-muted)', padding: '0.75rem 0.5rem' }}>{o.order_number.slice(-8)}</td>
                      <td style={{ fontWeight: 'bold', fontSize: '0.85rem', color: o.website_id === 'US' ? '#3b82f6' : '#10b981', padding: '0.75rem 0.5rem' }}>
                        {o.website_id === 'US' ? '🇺🇸 US' : '🇰🇷 KR'}
                      </td>
                      ...
                      <td className="fw-bold text-accent" style={{ fontSize: '0.9rem', padding: '0.75rem 0.5rem' }}>${o.total_amount.toLocaleString()}</td>
                    </tr>
                  ))
                )}
              </tbody>
                    ...
                    <tr key={p.id}>
                      ...
                      <td style={{ padding: '0.75rem 0.5rem' }}>
                        {p.stock_quantity === 0 ? (
                          ...
                          <span style={{ color: 'var(--warning)', fontWeight: 'bold', fontSize: '0.85rem' }}>Only {p.stock_quantity} left</span>
                        ) : (
                          <span style={{ color: 'var(--success)', fontSize: '0.85rem' }}>{p.stock_quantity} in stock</span>
                        )}
                      </td>
                    </tr>
                  ))
