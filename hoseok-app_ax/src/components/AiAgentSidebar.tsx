...
const agents = [
  ...
  },
  {
    ...
    desc: 'Monitors platform threats and blocks attacks autonomously.',
    avatar: '/agents/security_doberman.png',
    suggestions: ['🌊 Simulate DDoS attack', '💉 Run SQL Injection test', '🕷️ ZAP 취약점 진단 실행', '📊 Start AI Training Mode']
  }
];
...
export default function AiAgentSidebar() {
  ...
  const startAutoPilot = () => {
    ...
    setView('chat');
    setActiveAgent(null); // Global Orchestrator View
    
    // 1. 시스템 시작 - 메인 대시보드
    navigate('/');
    setMessages([{ role: 'system', type: 'text', content: '🚀 Auto-Pilot System Engaged. Overriding manual controls...' }]);
    
    setTimeout(() => {
      setMessages(prev => [...prev, { role: 'system', type: 'text', content: '🚨 Alert: Massive 10,000% traffic spike detected for LG OLED TV.' }]);
    }, 2000);

    // 2. 마케팅 요원 인지 및 어드민 작업 (3초)
    setTimeout(() => {
      setIsTyping(true);
      navigate('/products'); // 상품 관리 페이지에서 쿠폰 작업을 하는 연출
    }, 4000);

    setTimeout(() => {
      setIsTyping(false);
      setMessages(prev => [...prev, { role: 'agent', sender: 'Promotion Agent', type: 'text', content: 'To maximize conversion of this traffic, I am activating a 2-hour Flash Sale and issuing discount coupons on the admin panel.' }]);
    }, 7000);

    // 3. 마케팅 요원 반영 확인 (프론트엔드) (11초)
    setTimeout(() => {
      setIsTyping(true);
      navigate('/preview'); // 프론트엔드로 이동
    }, 11000);

    setTimeout(() => {
      setIsTyping(false);
      setMessages(prev => [...prev, { role: 'agent', sender: 'Promotion Agent', type: 'text', content: 'Admin update complete. Verifying that the Flash Sale banner is now visible on the live storefront.' }]);
      dispatchAiAction({ type: 'FLASH_SALE' });
      setFinancialImpact(prev => prev + 52000);
    }, 14000);

    // 4. 물류 요원 인지 및 어드민 작업 (17초)
    setTimeout(() => {
      setIsTyping(true);
      navigate('/orders'); // 주문 폭주를 보여주는 어드민 이동
    }, 17000);

    setTimeout(() => {
      setIsTyping(false);
      setMessages(prev => [...prev, { role: 'agent', sender: 'Order/Product Agent', type: 'text', content: '⚠️ Warning: Flash sale caused explosive orders. OLED TV stock is completely depleted. Modifying inventory status to "Pre-Order" in the admin pipeline.' }]);
    }, 20000);

    // 5. 물류 요원 반영 확인 (프론트엔드) (24초)
    setTimeout(() => {
      setIsTyping(true);
      navigate('/preview'); // 프론트엔드로 다시 복귀
    }, 24000);

    setTimeout(() => {
      setIsTyping(false);
      setMessages(prev => [...prev, { role: 'agent', sender: 'Order/Product Agent', type: 'text', content: 'Inventory modified. Verifying that the live storefront now correctly displays the "Pre-Order (10-15 Days)" button to customers.' }]);
      dispatchAiAction({ type: 'PRE_ORDER' });
      setFinancialImpact(prev => prev + 145000);
    }, 27000);

    // 6. 보안 요원 출동 (31초)
    setTimeout(() => {
      setIsTyping(true);
      navigate('/security'); // 보안 어드민으로 이동
    }, 31000);

    setTimeout(() => {
      setIsTyping(false);
      setMessages(prev => [...prev, { role: 'agent', sender: 'Security Guardian', type: 'text', content: '🚨 Alert: Detected 120 malicious bot checkout attempts targeting the Pre-Order stock. Permanently blocking the attacker IP subnets on the security firewall.' }]);
      fetch('http://localhost:4321/api/security', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'simulate', type: 'DDOS' })
      }).catch(()=>{});
      setFinancialImpact(prev => prev + 24000);
    }, 34000);

    setTimeout(() => {
      setMessages(prev => [...prev, { role: 'system', type: 'text', content: '✅ Auto-Pilot response complete. Threat neutralized and revenue secured.' }]);
      navigate('/');
    }, 37000);
  };
  ...
  const handleSelectAgent = (agent: any) => {
    ...
    if (agent.id === 'security') navigate('/security');
    else if (agent.id === 'order') navigate('/orders');
    else if (agent.id === 'product' || agent.id === 'promotion') navigate('/products');
    else if (agent.id === 'rollout') navigate('/preview');
    else navigate('/');
  };
  ...
  const submitMessage = async (text: string) => {
    ...
    if (activeAgent?.id === 'security') {
      ...
      if (text.includes('DDoS')) {
        setTimeout(() => {
          setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '🌊 Initiating DDoS Attack Simulation... Monitoring incoming traffic.' }]);
          setIsTyping(true);
          
          setTimeout(() => {
            setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '🚨 Alert: Massive influx of traffic detected from unknown IP ranges.' }]);
            setIsTyping(true);

            setTimeout(() => {
              setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '🔍 Analysis: The traffic is attempting to exhaust connection pools and bypass standard rate limiters.' }]);
              setIsTyping(true);

              setTimeout(() => {
                setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '🛡️ Action: Applying adaptive rate limiting and routing traffic through the Cloudflare scrubbing center.' }]);
                fetch('http://localhost:4321/api/security', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ action: 'simulate', type: 'DDOS' })
                ...
                setTimeout(() => {
                  setIsTyping(false);
                  setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '✅ Threat neutralized. Service is stable and all legitimate traffic is flowing normally.' }]);
                }, 2500);
              }, 3000);
            }, 3000);
          }, 2500);
        ...
        setTimeout(() => {
          setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '💉 Running SQL Injection Test on Search API...' }]);
          setIsTyping(true);

          setTimeout(() => {
            setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '🚨 Alert: Malicious payload detected: `\' OR 1=1; DROP TABLE users; --`' }]);
            setIsTyping(true);

            setTimeout(() => {
              setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '🔍 Analysis: Attempted data exfiltration and table drop via search input field.' }]);
              setIsTyping(true);

              setTimeout(() => {
                setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '🛡️ Action: Blocking the source IP immediately and sanitizing the input payload via WAF rules.' }]);
                fetch('http://localhost:4321/api/security', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ action: 'simulate', type: 'SQL_INJECTION' })
                });
                setIsTyping(true);

                setTimeout(() => {
                  setIsTyping(false);
                  setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '✅ Attack blocked. Database integrity verified.' }]);
                }, 2500);
              }, 3000);
            }, 3000);
          }, 2500);
        }, 1500);
      } else if (text.toUpperCase().includes('ZAP') || text.includes('진단') || text.includes('스캔')) {
        setTimeout(() => {
          setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '🕵️‍♂️ Initiating OWASP ZAP Baseline Scan across all production endpoints...' }]);
          setIsTyping(true);

          setTimeout(() => {
            setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '🕷️ ZAP Spidering complete. Active scanning for Top 10 vulnerabilities in progress...' }]);
            setIsTyping(true);

            setTimeout(() => {
              setIsTyping(false);
              setMessages(prev => [...prev, 
                { role: 'agent', type: 'text', content: '🚨 Scan complete. Detected a High-Risk Reflected XSS vulnerability in the global search parameter. I have automatically generated a temporary WAF rule to block this vector and created a Jira ticket for the Dev team.' },
                { role: 'agent', type: 'action', actionDetails: { type: 'OWASP_ZAP_REPORT' } }
              ]);
            }, 3000);
          }, 3000);
        }, 1500);
      } else if (text.includes('Training') || text.includes('학습') || text.includes('트레이닝')) {
        setTimeout(() => {
          setIsTyping(false);
          setMessages(prev => [...prev, { 
            role: 'agent', 
            type: 'text', 
            content: '🛡️ **Security Guardian Training Mode Initiated**\nConnecting to global threat intelligence feeds. I am now monitoring for abnormal traffic patterns. Please check the Security menu to see real-time blocking actions.' 
          }]);
        }, 1000);
      } else {
        // Fallthrough to Gemini API for open-ended questions
      }
      if (text.includes('DDoS') || text.includes('SQL') || text.toUpperCase().includes('ZAP') || text.includes('진단') || text.includes('스캔') || text.includes('Training') || text.includes('학습') || text.includes('트레이닝')) return;
    }
    ...
    try {
      ...
      if (data && data.text) {
        setMessages(prev => [...prev, { role: 'agent', type: 'text', content: data.text }]);
      } else {
        setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '응답을 생성하지 못했습니다.' }]);
      }
    } catch (err) {
      console.error("AI Fetch Error:", err);
      setIsTyping(false);
      setMessages(prev => [...prev, { role: 'agent', type: 'text', content: 'Connection to Gemini AI brain failed. Please check the network.' }]);
    }
  };
  ...
  return (
    <>
      <div className={`ai-sidebar-container ${isOpen ? 'open' : ''}`}>
        <div className="ai-header" style={{ position: 'relative' }}>
        <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <button 
            ...
            style={{ 
              background: isAutoPilot ? '#28a745' : '#e9ecef', 
              color: isAutoPilot ? '#fff' : '#495057', 
              border: 'none', 
              padding: '6px 12px', 
              borderRadius: '20px', 
              fontSize: '0.8rem', 
              fontWeight: 'bold',
              cursor: isAutoPilot ? 'default' : 'pointer',
              display: 'flex',
              alignItems: 'center',
              gap: '6px',
              transition: 'all 0.3s'
            }}
            title="Start Zero-Click Domino Scenario"
          >
      ...
      {view === 'dashboard' ? (
        <div className="ai-view-content">
          ...
          {agents.map(agent => (
            <div key={agent.id} className="ai-agent-item" style={{ display: 'flex', alignItems: 'center' }} onClick={() => handleSelectAgent(agent)}>
              <img src={agent.avatar} alt={agent.name} className="ai-agent-avatar" style={agent.id === 'security' ? { width: '74px', height: '74px' } : undefined} />
              <div className="ai-agent-info" style={{ flexGrow: 1 }}>
                <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {agent.name} {agent.id === 'security' && <span style={{ fontSize: '1.25em' }}>🛡️</span>}
                </h3>
                <p>{agent.desc}</p>
              </div>
              {agent.id === 'promotion' && (
        ...
        <>
          <div className="ai-view-content" style={{ display:'flex', flexDirection:'column' }}>
            <div className="ai-chat-header" style={{ position: 'relative' }}>
              <button className="back-btn" onClick={() => { setView('dashboard'); setIsAutoPilot(false); }}>←</button>
              {isAutoPilot ? (
                <>
                  <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: '#212529', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem' }}>🤖</div>
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{fontWeight:700, fontSize:'1.1rem'}}>Global Command Center</span>
                    <span style={{fontWeight:600, fontSize:'0.85rem', color: '#198754'}}>💰 AI Revenue Saved: ${financialImpact.toLocaleString()}</span>
                  </div>
                </>
              ) : (
                <>
            ...
            <div className="chat-history">
              {messages.map((msg, idx) => {
                const senderAgent = msg.sender ? agents.find(a => a.name.includes(msg.sender.split('/')[0])) : activeAgent;
                const avatar = senderAgent?.avatar || 'https://cdn-icons-png.flaticon.com/512/4712/4712035.png'; // fallback system icon
                return (
                  <div key={idx} style={{ display: 'flex', gap: '8px', marginBottom: '12px', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '100%' }}>
                    {msg.role === 'agent' && (
                      <div style={{ flexShrink: 0, width: '64px', height: '64px', borderRadius: '50%', background: '#fff', border: '1px solid #dee2e6', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                        <img src={avatar} alt="Agent Avatar" style={{ width: '80%', height: '80%', objectFit: 'contain' }} />
                      </div>
                    )}
                    <div className={`chat-bubble ${msg.role}`} style={{ margin: 0 }}>
                      {msg.sender && <div style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#0d6efd', marginBottom: '4px' }}>{msg.sender}</div>}
                    ...
                    <div className="action-card" style={{ borderLeft: '4px solid var(--accent)' }}>
                      ...
                      <div className="action-row">
                        ...
                        <button className="action-btn approve" style={{ background: 'var(--accent)' }} onClick={() => {
                          setMessages(prev => prev.filter(m => m !== msg));
                          setMessages(prev => [...prev, { role: 'user', type: 'text', content: 'Launch the site with Auto-SSL.' }]);
                          setIsTyping(true);
                          
                          setTimeout(() => {
                            setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '🔐 Contacting Let\'s Encrypt Authority via ACME protocol for DNS challenge...' }]);
                            setIsTyping(true);
                            
                            setTimeout(() => {
                              setIsTyping(false);
                              setMessages(prev => [...prev, 
                                { role: 'agent', type: 'text', content: `🚀 Deployment initiated! The **${msg.actionDetails.region}** storefront is now LIVE with HTTPS enabled.` },
                                { role: 'agent', type: 'action', actionDetails: { type: 'LETS_ENCRYPT_SUCCESS', domain: `lg.com/${msg.actionDetails.region.toLowerCase()}` } },
                  ...
                  {/* Let's Encrypt Success Card */}
                  {msg.type === 'action' && msg.actionDetails?.type === 'LETS_ENCRYPT_SUCCESS' && (
                    <div className="action-card" style={{ background: '#f8f9fa', border: '1px solid #198754', borderLeft: '4px solid #198754', marginTop: '8px' }}>
                      <div className="action-card-title" style={{ color: '#198754' }}>🔒 Let's Encrypt SSL Provisioned</div>
                      <div className="action-card-details" style={{ fontSize: '0.85rem' }}>
                        <p><strong>Domain:</strong> {msg.actionDetails.domain}</p>
                        <p><strong>Issuer:</strong> Let's Encrypt Authority X3</p>
                        <p><strong>Expiry:</strong> {new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]} (90 days)</p>
                        <p><strong>Status:</strong> 🟢 TLS 1.3 Active & WAF Encrypted</p>
                      </div>
                    </div>
                  )}

                  {/* OWASP ZAP Report Card */}
                  {msg.type === 'action' && msg.actionDetails?.type === 'OWASP_ZAP_REPORT' && (
                    <div className="action-card" style={{ background: '#fff', border: '1px solid #dc3545', borderLeft: '4px solid #dc3545', marginTop: '8px' }}>
                      <div className="action-card-title" style={{ color: '#dc3545' }}>⚠️ OWASP ZAP Scan Report</div>
                      <div className="action-card-details" style={{ fontSize: '0.85rem' }}>
                        <p><strong>Target:</strong> lg.com/global</p>
                        <p><strong>High Risk:</strong> 1 (Reflected XSS - /search?q=)</p>
                        <p><strong>Medium Risk:</strong> 3 (Missing Anti-clickjacking headers)</p>
                        <hr style={{ margin: '8px 0', borderColor: '#e9ecef' }} />
                        <p style={{ color: '#198754', fontWeight: 'bold' }}>🛡️ Action Taken:</p>
                        <ul style={{ paddingLeft: '20px', margin: '4px 0' }}>
                          <li>Injected WAF Rule: `Block regex &lt;script&gt;` on `q` param.</li>
                          <li>Jira Ticket Created: SEC-9412</li>
                        </ul>
                      </div>
                    </div>
                  )}
