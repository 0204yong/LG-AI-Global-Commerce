...
async function executeManageEntity(modelName, args) {
  ...
  try {
    if (action === 'READ') {
      ...
      if (id) {
        const updated = await db.update({ where: { id: BigInt(id) }, data });
      ...
      if (id) {
        const deleted = await db.delete({ where: { id: BigInt(id) } });
      ...
      if (modelName === 'orders') await generateDummyOrders();
  ...
  try {
    ...
    if (functionCalls && functionCalls.length > 0) {
      ...
      if (functionName === 'navigate') {
        ...
        const summary = {
          totalRevenue: Number(totalRevenueAggr._sum.total_amount) || 0,
        ...
        functionResponseData = await executeManageEntity('orders', args);
        ...
        functionResponseData = await executeManageEntity('admin_users', args);
        if (functionResponseData.requiresRefresh) actionData = { type: 'REFRESH_DATA' };
      }
...
async function generateDummyProducts() {
  const dummyProducts = [
    { name: "프리미엄 런닝화", price: 120000, stock_quantity: 50, description: "편안한 착화감을 자랑하는 최고급 런닝화", sku: 'RN-001', website_id: 'KR' },
    { name: "베이직 무지 티셔츠", price: 15000, stock_quantity: 200, description: "어디에나 잘 어울리는 기본 티셔츠", sku: 'TS-001', website_id: 'KR' },
    { name: "스마트 노이즈캔슬링 이어폰", price: 250000, stock_quantity: 5, description: "외부 소음을 완벽히 차단하는 스마트 이어폰", sku: 'EP-001', website_id: 'KR' },
    { name: "고급 가죽 지갑", price: 85000, stock_quantity: 15, description: "천연 소가죽으로 제작된 고급스러운 지갑", sku: 'WT-001', website_id: 'KR' },
    { name: "초경량 등산 배낭", price: 150000, stock_quantity: 8, description: "전문가를 위한 초경량 아웃도어 배낭", sku: 'BP-001', website_id: 'KR' }
  ];
